...
# Copyright (C) 2018, 2019, 2020, 2021  Stefan Vargyas
# 
...
youtube-data()
{
    ...
    local lrex='^PL([0-9a-zA-Z_-]{32}|[0-9A-F]{16})$|^(LL|UU)[0-9a-zA-Z_-]{22}$|^OL[0-9a-zA-Z_-]{39}$|^RD[0-9a-zA-Z_-]{13,}$'
    local crex='^UC[0-9a-zA-Z_-]{22}$'
    local vrex='^[0-9a-zA-Z_-]{11}$'
    local hrex='[0-9a-f]{8}'
    local chnx='playlists|videos|uploads'
    local lnkx="@(itself|$chnx)"
    ...
    local act="P"       # actions: \
                        ...
                        #   V: extract resource's content for the most recent NUM video items; the output produced is specified by the print options `-P', `--list', `--table', etc; `--[w]diff-{recent,uploads}' produce a 'diff' or 'wdiff' between previous version of the resource's cached content and the one specified by `--cache=SPEC'; the options `--relative-date', `--wrap-text' and `--color=auto' are implied if not overridden; NUM is at most the argument of `--max-results' and it is 5 by default; the `--*-recent' options are using Google's API 'search' endpoint, while the `--*-uploads' options are using the 'playlist-items' endpoint queried for channel's uploads playlist (--show-recent[=NUM],--diff-recent[=NUM],--wdiff-recent[=NUM],--show-uploads[=NUM],--diff-uploads[=NUM],--wdiff-uploads[=NUM])
    ...
    local r="channel"   # resource's type: 'channel', 'playlist' or 'video' (default: 'channel'); INPUT is either a file name or an id (default), determined by `--file' or `--id'; when INPUT terminates with '.json', consider it to be file name; when INPUT ends with '{,-playlists,-videos,-uploads}.json', set the linked type accordingly (--channel[=INPUT]|--playlist[=INPUT]|--video[=INPUT])
    ...
    local u=""          # pass `-u NUM' to 'diff' when action is `-D|--diff' or `-V|--diff-{recent,uploads}' (--unified=NUM)
    ...
    local V2=''         # argument of `-V|--{show,diff,wdiff}-{recent,uploads}'
    local V3
    ...
    while getopts "$opts" opt; do
        ...
        case "$opt" in
            ...
            V)	#!!! echo >&2 "!!! OPT='$OPT' OPTN='$OPTN' OPTARG='$OPTARG'"
                ...
                if [[ "${OPT:2}" == show-@(recent|uploads) ]]; then
                    [ "$act" != 'P' ] && {
                        act='P'
                        arg='+'
                    }
            ...
            r)	[[ -n "$OPT" || "$OPTARG" == $resx$resa ]] || {
                ...
                [ -n "$OPTARG" ] && {
                    if [[ "$OPTARG" =~ -($chnx)\.json$ ]]; then
                        # stev: take link type to be 'playlists'
                        # 'videos' or 'uploads' when the given 
                        # arg terminates with '-playlists.json',
                        # '-videos.json' or '-uploads.json'
                        # respectively
                        l="${BASH_REMATCH[1]}"
...
sed -ru '
    ...
    [ "$act" == 'P' ] && {
        [ -n "$V2" ] && {
            ...
            # the same argument goes for `--show-uploads' w.r.t. the first
            # 'playlist-items' 'uploads' page linked to the given channel
            local V3="${V2##*=}"
            [ -z "$V3" ] && V3=5
            local m2="$m"
            [ "$m2" == '+' ] && m2=50
